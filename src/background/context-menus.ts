...
export function initContextMenus() {
  prefs.getAndWatch('searches', async searches => {
    ...
    searches.forEach(search => {
      ...
      chrome.contextMenus.create(
        {
          id: `search-${search[SearchItemAlias.key]}`,
          contexts: ['selection'],
  ...
  chrome.contextMenus.onClicked.addListener((info, tab) => {
    ...
    if (menuItemId === 'add-page-to-home' && pageUrl && tab) {
      handleAddSite(tab.title || '', pageUrl);
      return;
    }
    if (menuItemId === 'add-to-home' && linkUrl) {
      handleAddSite(selectionText || '', linkUrl);
      return;
    }
    if (menuItemId.startsWith('search-') && selectionText) {
      const searchKey = menuItemId.substring(7);
